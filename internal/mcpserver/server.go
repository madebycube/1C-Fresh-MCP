...
type listProductCharacteristicsInput struct {
	ProductID string `json:"product_id,omitempty" jsonschema:"Optional product GUID"`
	Name      string `json:"name,omitempty" jsonschema:"Optional substring of a characteristic name"`
}

type listProductCharacteristicsOutput struct {
	Characteristics []service.ProductCharacteristic `json:"characteristics"`
	Count           int                             `json:"count"`
}
...
type listCurrenciesOutput struct {
	Currencies []service.Currency `json:"currencies"`
	Count      int                `json:"count"`
}
...
type getPriceDocumentInput struct {
	ID        string `json:"id" jsonschema:"Price document GUID from get_product_price or list_product_prices"`
	ProductID string `json:"product_id,omitempty" jsonschema:"Optional product GUID to filter lines"`
	Limit     int    `json:"limit,omitempty" jsonschema:"Maximum lines, 1 to 100; defaults to 20"`
...
func New(svc service.Service) *mcp.Server {
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: operations.ListProductCharacteristics.Tool, Description: operations.ListProductCharacteristics.Description,
		Annotations: &mcp.ToolAnnotations{ReadOnlyHint: true},
	}, func(ctx context.Context, _ *mcp.CallToolRequest, input listProductCharacteristicsInput) (*mcp.CallToolResult, listProductCharacteristicsOutput, error) {
		items, err := svc.ListProductCharacteristics(ctx, input.ProductID, input.Name)
		return nil, listProductCharacteristicsOutput{Characteristics: items, Count: len(items)}, err
	})
	mcp.AddTool(server, &mcp.Tool{
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: operations.ListCurrencies.Tool, Description: operations.ListCurrencies.Description,
		Annotations: &mcp.ToolAnnotations{ReadOnlyHint: true},
	}, func(ctx context.Context, _ *mcp.CallToolRequest, _ listCurrenciesInput) (*mcp.CallToolResult, listCurrenciesOutput, error) {
		items, err := svc.ListCurrencies(ctx)
		return nil, listCurrenciesOutput{Currencies: items, Count: len(items)}, err
	})
	mcp.AddTool(server, &mcp.Tool{
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: operations.GetPriceDocument.Tool, Description: operations.GetPriceDocument.Description,
		Annotations: &mcp.ToolAnnotations{ReadOnlyHint: true},
	}, func(ctx context.Context, _ *mcp.CallToolRequest, input getPriceDocumentInput) (*mcp.CallToolResult, service.PriceDocument, error) {
		limit := input.Limit
		if limit == 0 {
			limit = 20
		}
